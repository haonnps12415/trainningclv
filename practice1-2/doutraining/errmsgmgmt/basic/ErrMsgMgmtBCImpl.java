...
public class ErrMsgMgmtBCImpl extends BasicCommandSupport implements ErrMsgMgmtBC {
	 ...
	 */
	public List<ErrMsgVO> ErrMsgVOSearch(ErrMsgVO errMsgVO) throws EventException {
		try {
			return dbDao.ErrMsgVOSearch(errMsgVO);
	 ...
	 * @exception EventException
	 */
	public void ErrMsgVOMulti(ErrMsgVO[] errMsgVO, SignOnUserAccount account) throws EventException{
		try {
			...
			if ( insertVoList.size() > 0 ) {
				dbDao.addErrMsgVOMultiS(insertVoList);
			}
			
			if ( updateVoList.size() > 0 ) {
				dbDao.modifyErrMsgVOMultiS(updateVoList);
			}
			
			if ( deleteVoList.size() > 0 ) {
				dbDao.removeErrMsgVOMultiS(deleteVoList);
			}
		} catch(DAOException ex) {
			throw new EventException(new ErrorHandler(ex).getMessage(),ex);
		} catch (Exception ex) {
